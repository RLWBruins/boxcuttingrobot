...
{
  bool succeeded = true;
  if (surface_detection_.find_surfaces()) // Deze functie staat in het bestand surface_detection.cpp
  {
    ...
    // ORIGINAL GODEL CODE : ROS_ASSERT(meshes.size() == surface_clouds.size()); [For the box-cutting project : this part is commented because we only select the surface cloud with the most points]
