...
bool RobotScan::generate_scan_display_trajectory(moveit_msgs::DisplayTrajectory& traj_data)
{
  ...
  if (create_scan_trajectoryImproved(poses, robot_traj))
  {
...
bool RobotScan::generate_scan_poses(geometry_msgs::PoseArray& poses)
{
  ...
  bool succeeded = true;
  moveit_msgs::RobotTrajectory robot_traj;
  succeeded = create_scan_trajectoryImproved(poses.poses, robot_traj);
...
int RobotScan::scan(bool move_only)
{
  ...
  int poses_reached = 0;
  moveit_msgs::RobotTrajectory robot_traj;
  if (create_scan_trajectoryImproved(scan_traj_poses_, robot_traj))
  {
...
//      move_group_ptr_->setPoseTarget(trajectory_poses[i], params_.tcp_frame);
      ...
      if (!move_only)
      {
        ...
        if (msg)
        {
          ...
          // Let the node point_cloud_generator_node know that it is time to make a 3D screenshot :
          ros::NodeHandle nh("/");
...
bool RobotScan::create_scan_trajectoryImproved(std::vector<geometry_msgs::Pose>& scan_poses,
