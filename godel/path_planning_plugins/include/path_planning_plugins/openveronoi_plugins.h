...
namespace openveronoi
{
  ...
  public:
    ...
    void init(pcl::PolygonMesh mesh);
    bool generatePath(std::vector<geometry_msgs::PoseArray>& path);
    float calculateAngleCorrection (std::unique_ptr<mesh_importer::MeshImporter>& mesh_importer_ptr, const pcl::PolygonMesh& input_mesh);
    double calculateDistance(double x2, double x1, double y2, double y1);
  };
}
}
