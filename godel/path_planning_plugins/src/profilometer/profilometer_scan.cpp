...
static const double GROWTH_FACTOR = 1.2;
                                          ...
                                          double overlap)
{
  ...
  // ORIGINAL GODEL CODE : n_slices = static_cast<int>(bbox.h / (adjusted_width)) + 1;
  n_slices = 1; // For the box-cutting project: by setting n_slices=1 there will always be just one slice (path) that goes exactly through the middle of the rectangle.
...
generateProfilometerScanPath(const Boundary& boundary, const PlanningParams& params)
{
  ...
  //ORIGINAL GODEL CODE : std::vector<RotatedRect> slices = sliceBoundingBox(bbox, params.scan_width, params.overlap);
  std::vector<RotatedRect> slices = sliceBoundingBox(bbox, bbox.h, params.overlap); // Edited for the box-cutting project
